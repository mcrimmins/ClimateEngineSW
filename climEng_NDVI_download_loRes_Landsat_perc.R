...
#lapply(query, write, paste0(home,"/",dataDir,"/query.txt"), append=TRUE)
capture.output(query, file = paste0(home,"/",dataDir,"/query.txt"))
...
# Make API request
resp <- request(base_url = paste0(root_url, endpoint)) %>%
...
task_id = resp %>%
  resp_body_json() %>%
  pluck('task_response', 'id')

print(paste0("GEE task ID: ",task_id))
